...
> 検索の再ランキングが使うドメイン語彙（除外語・意図判定・加点する語形）も同ファイルの
> `[lexical]` 節にあり、`src/rag` は特定ナレッジの語を持ちません。
...
| 7 | ナレッジ設定分離 | `config/knowledge.toml` で本文・出典・評価セット・collection・検索語彙（`[lexical]`）を管理 |
